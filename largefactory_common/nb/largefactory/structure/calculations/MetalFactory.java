    ...
    public static void learnMetal(NodeList metal){
        ...
        for(int k = 1; k < metal.getLength(); k++){
            ...
            if(metal.item(k).getNodeName() == "yield"){
                a.importYieldValue(metal.item(k).getAttributes().getNamedItem("material").getNodeValue(), Float.parseFloat(metal.item(k).getTextContent()));
                //this is primary yield so not hidden as slag
            }else if(metal.item(k).getNodeName() == "primary"){
                a.importPrimaryValue(metal.item(k).getAttributes().getNamedItem("primary").getNodeValue(), Float.parseFloat(metal.item(k).getTextContent()));
                //error catch
            }else{
