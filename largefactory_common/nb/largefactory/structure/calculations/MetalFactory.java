    ...
    public static void learnMetal(NodeList metal) {
        ...
        for (int k = 1; k < metal.getLength(); k++) {
            ...
            if (metal.item(k).getNodeName() == "yield") {
                a.importYieldValue(
                        metal.item(k).getAttributes().getNamedItem("material")
                        .getNodeValue(),
                ...
                a.importPrimaryValue(metal.item(k).getAttributes()
                        .getNamedItem("primary").getNodeValue(),
                        Float.parseFloat(metal.item(k).getTextContent()));
                // error catch
            } else {
                System.out.println("The material "
                        + metal.item(0).getTextContent()
                        + " has an improper yield");
            }
        }
